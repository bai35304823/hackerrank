...
//    Interface representing the environment in which the current application is running
	...
	public Booking addBooking(@RequestBody Booking booking) {
		...
		booking.setPassengers(passengersRepository.findByPassengerID(booking.getPassengerId()));
		booking.setFlight(flightRepository.findByScheduleId(booking.getScheduleId()));
	...
	private void validateParams(Booking booking) {
		...
		if (Objects.isNull(passengersRepository.findByPassengerID(booking.getPassengerId()))) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "The passenger cannot be found!");
		}

		if (Objects.isNull(flightRepository.findByScheduleId(booking.getScheduleId()))) {
