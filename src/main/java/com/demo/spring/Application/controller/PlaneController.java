...
//    Interface representing the environment in which the current application is running
    ...
    public Plane addPlane(@RequestBody Plane plane) {
    	...
    	plane_db.setPlaneRegNo(new StringBuffer(environment.getProperty("plane.prefix")).append(
    			StringUtils.leftPad(String.valueOf(plane_db.getId()), 4, "0")).toString());
    ...
    public ResponseEntity<String> deletePlane(@RequestBody Plane plane) {
    	...
    	return ResponseEntity.status(HttpStatus.OK)
    	        .body("Delete success for Plane " + plane.getPlaneRegNo());
    }
    ...
    private void validateParams(String planeCapacity, String maker, String model) {
		if (StringUtils.isEmpty(planeCapacity) || StringUtils.isEmpty(maker) || StringUtils.isEmpty(model)) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The plane maker, model, and seating capacity" + 
					" must be provided");
		}

		if (!Stream.of("50","100","150","500").anyMatch(str -> StringUtils.equalsIgnoreCase(str, planeCapacity))) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The seating capacity should be 50,100,150 or 500.");
		}
    ...
    private void validateParams(String planeRegNo) {
		if (StringUtils.isEmpty(planeRegNo) ) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The plane registration number" + 
					" must be provided");
		}
