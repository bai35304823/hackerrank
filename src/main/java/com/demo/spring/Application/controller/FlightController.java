...
//    Interface representing the environment in which the current application is running
	...
	public Flight addFlight(@RequestBody Flight flight) {
		...
		flight.setDepartCountry(countryRepository.findByCountryId(flight.getDepartCountryId()));
		flight.setDepartAirport(airportRepository.findByAirportCode(flight.getDepartAirportCode()));
		flight.setDestCountry(countryRepository.findByCountryId(flight.getDestCountryId()));
		flight.setDestAirport(airportRepository.findByAirportCode(flight.getDestAirportCode()));
		flight.setPlane(planeRepository.findByPlaneRegNo(flight.getPlaneRegNo()));
	...
	public List<Flight> getByPlaneRegNo(@PathVariable(value = "planeRegNo") String planeRegNo) {
		Plane plane = planeRepository.findByPlaneRegNo(planeRegNo);
	...
	public List<Flight> getBySourceAndDestination(@PathVariable(value = "source") String source,
		...
		return flightRepository.findByDepartCountryAndDestCountry(countryRepository.findByCountryId(source),
				countryRepository.findByCountryId(destination));
	}
	...
	public Flight updateByScheduleNumber(@RequestBody Flight flight) {
		...
		flightDB.setDepartCountry(countryRepository.findByCountryId(flight.getDepartCountryId()));
		flightDB.setDepartAirport(airportRepository.findByAirportCode(flight.getDepartAirportCode()));
		flightDB.setDestCountry(countryRepository.findByCountryId(flight.getDestCountryId()));
		flightDB.setDestAirport(airportRepository.findByAirportCode(flight.getDestAirportCode()));
