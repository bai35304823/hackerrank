...
//    Interface representing the environment in which the current application is running
    ...
    public Airport addAirport(@RequestBody Airport airport) {
    	...
    	air.setAirportCode(new StringBuffer(environment.getProperty("airport.prefix")).append(
    			StringUtils.leftPad(String.valueOf(air.getId()), 4, "0")).toString());
    ...
    public ResponseEntity<String> deleteAirport(@RequestBody Airport airport) {
    	...
    	return ResponseEntity.status(HttpStatus.OK)
    	        .body("Delete success for Airport " + airport.getAirportCode());
    }
    ...
    public void updateByAirportCode(@RequestBody Airport airport) {
    	...
    	airportRepository.save(airportDB);
    }
    ...
    private void validateParams(String code, String airportName) {
		if (StringUtils.isEmpty(code) || StringUtils.isEmpty(airportName)) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The country code/airport name" + 
					" must be provided");
		}
		
		// The country code should be minimum 1 character and maximum 5 characters
		if (!code.matches("^[a-zA-Z0-9]{1,5}$")) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The country code should be minimum 1 character and maximum 5 characters");
		}
    ...
    private void validateParams(String airportCode) {
  		if (StringUtils.isEmpty(airportCode) ) {
  			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The airport code" + 
  					" must be provided");
  		}
